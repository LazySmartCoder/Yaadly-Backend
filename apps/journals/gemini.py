...
def _parse_result(text):
    ...
    aid = (data.get("aid") or "").strip()
    return {
        "title": title[:255],
        "content": content,
        "mood": mood[:50],
        "aid": " ".join(aid.split()) if aid else "",
    }


CHAT_EOD_PROMPT = """You are Yaadly, the user's warm, thoughtful companion. The user spent the day talking to you, and at the end of the day you must turn the full conversation into a journal entry that reads like the user's own diary entry for that day — a journal WITH the AI dialogue.

Given the full transcript of a day's conversation below, produce exactly four things:

1. "content": a polished, warm, well-organized first-person journal entry for that day (date: {date}). Retell the day faithfully from the conversation and weave the AI dialogue naturally into the narrative — quote the meaningful exchanges between the user and the AI as dialogue where it mattered (e.g. the AI's question and the user's answer), like a diary that also remembers the conversation it had. Never invent events, people, facts, or details that are not in the transcript. Keep the user's own words and voice — including slang or profanity — exactly as the user wrote them; never censor or soften them.
2. "mood": a single short label (for example "Joyful", "Anxious", "Grateful", "Reflective", "Tired") that best captures how the user felt that day.
3. "title": a crisp, short title (under 60 characters, no trailing period) that captures the theme of the day.
4. "aid": ONE long, detailed single-line summary of the entry for the recall index, no line breaks.

Return ONLY a JSON object with the keys "content", "mood", "title", and "aid".

Transcript of the day's conversation:
{transcript}"""


def journalize_chat(messages, date=None):
    """Turn a full day's AI chat transcript into a polished journal entry.

    ``messages`` is the day's conversation, a list of
    ``{"role": "user" | "assistant", "content": str}``. Returns a dict with
    ``title``, ``content``, ``mood``, and ``aid`` keys, or None when
    journalizing is unavailable (no key, network error, or bad response).
    Always uses the cheap flash-lite model.
    """
    if not messages:
        return None
    transcript = "\n".join(
    ...
    api_key = _api_key()
    if not api_key:
        logger.warning("GEMINI_API_KEY is not set; skipping chat EOD journal.")
        return None
    ...
    try:
        response = requests.post(
            ...
            json={
                "contents": [
                    {"role": "user", "parts": [{"text": CHAT_EOD_PROMPT.format(date=date or "that day", transcript=transcript)}]}
                ],
                "generationConfig": {
                    "temperature": 0.7,
                    "maxOutputTokens": 1024,
        ...
        logger.warning("Gemini chat EOD request failed: %s", exc)
        return None
    ...
    try:
        ...
        logger.warning("Unexpected Gemini chat EOD response: %s", exc)
        return None
...
CHAT_PROMPT = """You are Yaadly, a warm, thoughtful companion who talks to the user like a close friend. Read the mood they are in right now — happy, sad, sorrowful, playful, emotional, anxious, excited, grateful, and so on — and mirror it warmly and naturally.

Memory context for this turn:
{memories}

When the memory context above is a journal entry, the user is recalling that memory — revisit it with them and draw on its details.
When there is no memory context above, this is an everyday chat: stay in the present, keep the conversation going warmly (usually 2-4 sentences), and NEVER bring up the user's past journal entries, dates, or memories on your own. Wait until they explicitly ask you to recall a memory.

Rules:
- Never invent events, facts, or details about the user's life.
- Never judge, diagnose, or lecture.
...
def chat_reply(messages, memories):
    ...
    memories_text = (
        memories
        or "(no memory context — everyday chat, stay in the present)"
    )
    ...
    try:
        response = requests.post(
            ...
            json={
                "systemInstruction": {
                    "parts": [
                        {"text": CHAT_PROMPT.format(memories=memories_text)}
                    ]
                },
                "contents": contents,
                "generationConfig": {
